...
const (
	codeUniqueViolation = "23505" // UNIQUE: email duplicado
	codeCheckViolation  = "23514" // CHECK / RAISE del trigger de 5 perfiles
)
...
func (r *PostgresUsuarioRepo) CrearUsuarioConPerfilInicial(
	ctx context.Context, u *domain.Usuario, nombrePerfil string,
) (usuarioID, perfilID string, err error) {
	...
	err = tx.QueryRow(ctx,
		`INSERT INTO usuarios (email, password_hash, oauth_proveedor, rol_base)
		 VALUES ($1, $2, $3, $4) RETURNING id`,
		u.Email, u.PasswordHash, u.OAuthProveedor, string(u.RolBase),
	).Scan(&usuarioID)
	if err != nil {
	...
	err = tx.QueryRow(ctx,
		`INSERT INTO perfiles (usuario_id, nombre) VALUES ($1, $2) RETURNING id`,
		usuarioID, nombrePerfil,
	).Scan(&perfilID)
	if err != nil {
		return "", "", err
	}
...
func (r *PostgresUsuarioRepo) CrearPerfil(
	...
	var id string
	err := r.db.QueryRow(ctx,
		`INSERT INTO perfiles (usuario_id, nombre, es_infantil, idioma)
		 VALUES ($1, $2, $3, $4) RETURNING id`,
		p.UsuarioID, p.Nombre, p.EsInfantil, p.Idioma,
	).Scan(&id)
	if err != nil {
...
func (r *PostgresUsuarioRepo) ListarPerfiles(
	ctx context.Context, usuarioID string,
) ([]domain.Perfil, error) {

	rows, err := r.db.Query(ctx,
		`SELECT id, usuario_id, nombre, es_infantil, idioma
		 FROM perfiles WHERE usuario_id = $1 ORDER BY nombre`,
		usuarioID,
	)
	if err != nil {
		return nil, err
	...
	for rows.Next() {
		var p domain.Perfil
		if err := rows.Scan(&p.ID, &p.UsuarioID, &p.Nombre, &p.EsInfantil, &p.Idioma); err != nil {
			return nil, err
		}
