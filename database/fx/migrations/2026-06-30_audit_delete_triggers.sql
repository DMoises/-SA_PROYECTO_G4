-- Migracion: habilita auditoria de DELETE en fx-db (TAREA 1).
...
-- arranque (directorio de datos vacio). En una fx-db que YA existe, los
...
--
-- Como aplicarla:
--   Docker (VM de BD):  docker exec -i quetxal-fx-db \
--                         psql -U "$FX_DB_USER" -d "$FX_DB_NAME" \
--                         < database/fx/migrations/2026-06-30_audit_delete_triggers.sql
--
--   BD externa (Fase 3, VM 10.10.0.10): usar psql -h 10.10.0.10 -p <puerto-fx>
--   con $FX_DB_USER / $FX_DB_NAME y -f sobre este archivo.
-- ------------------------------------------------------------------------
