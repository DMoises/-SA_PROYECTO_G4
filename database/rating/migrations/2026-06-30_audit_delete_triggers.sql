-- Migracion: habilita auditoria de DELETE en rating-db (TAREA 1).
...
-- arranque (directorio de datos vacio). En una rating-db que YA existe, los
...
--
-- Como aplicarla:
--   Docker (VM de BD):  docker exec -i quetxal-rating-db \
--                         psql -U "$RATING_DB_USER" -d "$RATING_DB_NAME" \
--                         < database/rating/migrations/2026-06-30_audit_delete_triggers.sql
--
--   BD externa (Fase 3, VM 10.10.0.10): usar psql -h 10.10.0.10 -p <puerto-rating>
--   con $RATING_DB_USER / $RATING_DB_NAME y -f sobre este archivo.
-- ------------------------------------------------------------------------
