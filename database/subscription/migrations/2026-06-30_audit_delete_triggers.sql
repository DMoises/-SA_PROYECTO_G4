-- Migracion: habilita auditoria de DELETE en subscription-db (TAREA 1).
...
--
-- Por que: el DDL de triggers de 03_audit.sql solo corre en el primer
-- arranque (directorio de datos vacio). En una subscription-db que YA existe, los
...
--
-- Como aplicarla:
--   Docker (VM de BD):  docker exec -i quetxal-subscription-db \
--                         psql -U "$SUBSCRIPTION_DB_USER" -d "$SUBSCRIPTION_DB_NAME" \
--                         < database/subscription/migrations/2026-06-30_audit_delete_triggers.sql
--
--   BD externa (Fase 3, VM 10.10.0.10): usar psql -h 10.10.0.10 -p <puerto-subscription>
--   con $SUBSCRIPTION_DB_USER / $SUBSCRIPTION_DB_NAME y -f sobre este archivo.
-- ------------------------------------------------------------------------
